...
m.register 'initialized' {
    ---@async
    function(params)
        local prog = progress.create(workspace.getFirstScope().uri, lang.script.WINDOW_INITIALIZING, 0.5)
...
m.register 'textDocument/hover' {
    ...
    function(params)
        ...
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_HOVER, 0.5)
        return defer(prog, function()
...
m.register 'textDocument/definition' {
    ...
    function(params)
        ...
        workspace.awaitReady(uri)
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_DEFINITION, 0.5)
        return defer(prog, function()
            local state = files.getState(uri)
            if not state then
                return nil
...
m.register 'textDocument/typeDefinition' {
    ...
    function(params)
        local uri = files.getRealUri(params.textDocument.uri)
        workspace.awaitReady(uri)
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_TYPE_DEFINITION, 0.5)
        return defer(prog, function()
            local state = files.getState(uri)
            if not state then
                return
...
m.register 'textDocument/references' {
    ...
    function(params)
        local uri = files.getRealUri(params.textDocument.uri)
        workspace.awaitReady(uri)
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_REFERENCE, 0.5)
        return defer(prog, function()
            local state = files.getState(uri)
            if not state then
                return nil
...
m.register 'textDocument/rename' {
    ...
    function(params)
        local uri = files.getRealUri(params.textDocument.uri)
        workspace.awaitReady(uri)
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_RENAME, 0.5)
        return defer(prog, function()
            local state = files.getState(uri)
            if not state then
                return nil
...
m.register 'textDocument/completion' {
    ...
    function(params)
        ...
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_COMPLETION, 0.5)
        return defer(prog, function()
            ...
            local zoneEnd = tracy.ZoneEnd
            return defer(zoneEnd, function()
                local easy  = false
                local items = {}
...
m.register 'textDocument/signatureHelp' {
    ...
    function(params)
        ...
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_SIGNATURE, 0.5)
        return defer(prog, function()
            local pos     = converter.unpackPosition(state, params.position)
...
m.register 'textDocument/documentSymbol' {
    ...
    function(params)
        local uri = files.getRealUri(params.textDocument.uri)
        workspace.awaitReady(uri)
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_SYMBOL, 0.5)
        return defer(prog, function()
            local state = files.getState(uri)
            if not state then
                return nil
...
m.register 'workspace/symbol' {
    ...
    ---@async
    function(params)
        local prog = progress.create(workspace.getFirstScope().uri, lang.script.WINDOW_PROCESSING_WS_SYMBOL, 0.5)
        return defer(prog, function()
        ...
        if not client.getOption('useSemanticByRange') then
            m.register 'textDocument/semanticTokens/full' {
                ...
                function(params)
                    ...
                    workspace.awaitReady(uri)
                    await.sleep(0.0)
                    local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_SEMANTIC_FULL, 0.5)
                    return defer(prog, function()
...
m.register 'textDocument/semanticTokens/range' {
    ...
    function(params)
        ...
        local uri = files.getRealUri(params.textDocument.uri)
        workspace.awaitReady(uri)
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_SEMANTIC_RANGE, 0.5)
        return defer(prog, function()
            await.sleep(0.0)
            local state = files.getState(uri)
            if not state then
...
m.register 'textDocument/onTypeFormatting' {
    ...
    function(params)
        local uri = files.getRealUri(params.textDocument.uri)
        workspace.awaitReady(uri)
        local prog = progress.create(uri, lang.script.WINDOW_PROCESSING_TYPE_FORMATTING, 0.5)
        return defer(prog, function()
            local ch    = params.ch
            local state = files.getState(uri)
            if not state then
