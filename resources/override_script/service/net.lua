...
local function fd_set_read(s)
    if bit.band(s._flags, SELECT_READ) ~= 0 then
        return
    end
    s._flags = bit.bor(s._flags, SELECT_READ)
...
local function fd_clr_read(s)
    ...
    s._flags = bit.band(s._flags, bit.bnot(SELECT_READ))
    selector:event_mod(s._fd, s._flags)
end

local function fd_set_write(s)
    if bit.band(s._flags, SELECT_WRITE) ~= 0 then
        return
    end
    s._flags = bit.bor(s._flags, SELECT_WRITE)
    selector:event_mod(s._fd, s._flags)
end
...
local function fd_clr_write(s)
    ...
    s._flags = bit.band(s._flags, bit.bnot(SELECT_WRITE))
    selector:event_mod(s._fd, s._flags)
end
...
local function update_stream(s, event)
    if bit.band(event, SELECT_READ) ~= 0 then
    ...
    if bit.band(event, SELECT_WRITE) ~= 0 then
    ...
    selector:event_add(fd, SELECT_WRITE, function()
        ...
        if ok then
            ...
            if s._writebuf ~= "" then
                ...
                if s._writebuf ~= "" then
                    s._flags = bit.bor(SELECT_READ, SELECT_WRITE)
                else
