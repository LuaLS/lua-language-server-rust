...
local function be_tochar(code)
    return strchar(band(rshift(code, 8), 0xFF), band(code, 0xFF))
end

local function be_tobyte(s, i)
    local h, l = strbyte(s, i, i+1)
    return bor(lshift(h, 8), l)
end

local function le_tochar(code)
    return strchar(band(code, 0xFF), band(rshift(code, 8), 0xFF))
end
...
local function utf16char(tochar, code)
    if code < 0x10000 then
        ...
        code = code - 0x10000
        return tochar(0xD800 + rshift(code, 10))..tochar(0xDC00 + band(code, 0x3FF))
    end
end
...
local function utf16next(s, n, tobyte)
    ...
    elseif code1 >= 0xD800 and code1 < 0xDC00 then
        ...
        local code = 0x10000 + lshift((code1 - 0xD800), 10) + band((code2 - 0xDC00), 0x3FF)
        return n+2, code
    else
        return n+2 --invaild
    end
...
local function utf8next(s, n)
    ...
    -- CppCXY luajit has bug for \0
    if strmatch(s, "^[\1-\x7F]", n) then
