...
local MODIFY = bit.lshift(1, 0)
local RENAME = bit.lshift(1, 1)
...
function m.update()
    ...
    for _, watching in pairs(m._watchings) do
        ...
        for _ = 1, 10000 do
            ...
            if ev == 'modify' then
                collect[path] = bit.bor(collect[path] or 0, MODIFY)
            elseif ev == 'rename' then
                collect[path] = bit.bor(collect[path] or 0, RENAME)
            end
        end
    end
